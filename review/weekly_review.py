...
from core.agent_data import load_agent_data
import json
...
def run_account(account_id: str, account_data: dict, quotes: dict, date: str, week_range: str):
    ...
    # 7. 生成报告
    md_content = render_report_md(account_name, positions, all_signals, index_data, decision, ai_section, date, turnover, agent_data=load_agent_data(date))
    html_content = render_report_html(account_name, positions, all_signals, index_data, decision, ai_section, date, account_data, turnover, agent_data=load_agent_data(date))
